...
async function sendBrevoEmail({ to, subject, htmlContent, replyTo = null }) {
    try {
        ...
        console.error('❌ Brevo email error:', {
            message: getBrevoErrorInfo(error).message,
            response: undefined,
            status: getBrevoErrorInfo(error).status,
            code: getBrevoErrorInfo(error).code,
            brevoMessage: getBrevoErrorInfo(error).brevoMessage
        });
        throw error;
    }
}

function getBrevoErrorInfo(error) {
    const responseData = error.response?.body || error.response?.text || error.response?.data;
    ...
    return {
        ...
        brevoMessage: parsedResponse?.message
    };
}

function getContactEmailFailureMessage(error) {
    const brevoError = getBrevoErrorInfo(error);
    const brevoMessage = String(brevoError.brevoMessage || '').toLowerCase();

    if (brevoError.status === 401 && brevoMessage.includes('unrecognised ip address')) {
        return 'Email service blocked this server IP. Add this IP to Brevo authorized IPs or disable authorized IP restriction.';
    }

    if (brevoError.status === 401) {
        return 'Email service authentication failed. Please check the Brevo API key.';
    }
...
<body>
    ...
    <div class="footer">
        ...
        <p style="margin-top: 20px;">© 2026 Raju IT. All rights reserved.</p>
    </div>
</body>
</html>`;
}
...
app.post("/send", async (req, res) => {
    ...
    const emailHtml = `
        <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;">
            <div style="background-color: white; padding: 20px; border-radius: 5px;">
                ...
                <hr style="border: 1px solid #eee;">
                <h3>Message:</h3>
                <p style="background-color: #f9f9f9; padding: 15px; border-left: 3px solid #ffc800;">${safeMessage}</p>
                <hr style="border: 1px solid #eee;">
    ...
    try {
        ...
        console.error("Contact form Brevo error:", getBrevoErrorInfo(error));
        res.status(500).json({
            success: false,
            message: getContactEmailFailureMessage(error)
        });
    }
});
