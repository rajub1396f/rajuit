...
const { sendInvoiceEmail } = require('./invoice-email-service');
...
app.get("/get-orders", verifyToken, async (req, res) => {
  try {
    ...
    // Get all orders for the user first - includes all fields for website/app sync
...
// Send invoice email (on demand - button click)
app.post("/send-invoice-email/:orderId", verifyToken, async (req, res) => {
  try {
    const userId = req.user?.id;
    const orderId = req.params.orderId;
    console.log(`ðŸ"§ POST /send-invoice-email/${orderId} - User: ${userId}`);
    ...
    // Get order details
    const orderResult = await sql`
      SELECT o.id, o.user_id, o.total_amount, o.invoice_pdf_url,
             u.name, u.email
      FROM orders o JOIN users u ON o.user_id = u.id
      WHERE o.id = ${orderId} AND o.user_id = ${userId}
    `;
    ...
    if (!order.invoice_pdf_url) {
      return res.status(400).json({ 
        success: false,
        message: "Invoice PDF not yet generated. Please try again in a moment."
      });
    }
    ...
    try {
      ...
      console.log(`âœ… Invoice email sent to ${user.email}`);
      res.json({ 
        success: true, 
...
app.post("/place-order", verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.log(`\nðŸ›' PLACE ORDER REQUEST`);
    console.log(`ðŸ'¤ User ID: ${userId}`);
    console.log(`ðŸ"¦ Items: ${items?.length}`);
    console.log(`ðŸ'° Total: à§³${totalAmount}`);
    console.log(`ðŸ'¤ Shipping Name: ${shippingName}`);
    console.log(`ðŸ"± Phone: ${shippingPhone}`);
    console.log(`ðŸš  Address: ${shippingAddress}`);
    ...
    if (!shippingAddress || shippingAddress.trim() === '') {
      console.log("âŒ Shipping address required");
      return res.status(400).json({ 
        success: false, 
    ...
    if (!shippingName || shippingName.trim() === '') {
      console.log("âŒ Shipping name required");
      return res.status(400).json({ 
        success: false, 
    ...
    if (!shippingPhone || shippingPhone.trim() === '') {
      console.log("âŒ Shipping phone required");
      return res.status(400).json({ 
        success: false, 
    ...
    const user = userResult[0];
    console.log(`ðŸ'¤ Order for: ${user.name} (${user.email})`);
    ...
    console.log(`\nâœ… ORDER PLACED SUCCESSFULLY - ID: #${orderId}\n`);
    res.status(201).json({
      success: true,
      message: "Order placed successfully",
      orderId: orderId,
    ...
    setImmediate(async () => {
      try {
        console.log(`ðŸ"§ Inserting ${items.length} items for order #${orderId} in background...`);
        for (const item of items) {
        ...
        console.log(`âœ… All items inserted for order #${orderId}`);
      } catch (itemErr) {
        console.error(`âŒ Error inserting items for order #${orderId}:`, itemErr);
      }
    });
...
app.get("/get-order/:orderId", verifyToken, async (req, res) => {
  try {
    ...
    console.log(`âœ… Order found with ${itemsResult.length} items`);
...
// Admin: Setup Neon Database Schema (run once)
app.post("/admin/setup-database", async (req, res) => {
  try {
    ...
    console.log(`\nðŸ"§ Setting up Neon database schema...`);
    ...
    // Add individual shipping details columns
    await sql`
      ALTER TABLE orders 
      ADD COLUMN IF NOT EXISTS shipping_name VARCHAR(255)
    `;
    console.log(`âœ… Added shipping_name column`);

    await sql`
      ALTER TABLE orders 
      ADD COLUMN IF NOT EXISTS shipping_phone VARCHAR(20)
    `;
    console.log(`âœ… Added shipping_phone column`);

    await sql`
      ALTER TABLE orders 
      ADD COLUMN IF NOT EXISTS shipping_city VARCHAR(100)
    `;
    console.log(`âœ… Added shipping_city column`);

    await sql`
      ALTER TABLE orders 
      ADD COLUMN IF NOT EXISTS shipping_state VARCHAR(100)
    `;
    console.log(`âœ… Added shipping_state column`);

    await sql`
      ALTER TABLE orders 
      ADD COLUMN IF NOT EXISTS shipping_postal VARCHAR(20)
    `;
    console.log(`âœ… Added shipping_postal column`);

    await sql`
      ALTER TABLE orders 
      ADD COLUMN IF NOT EXISTS shipping_country VARCHAR(100)
    `;
    console.log(`âœ… Added shipping_country column`);
    ...
    `;
    console.log(`âœ… Created index on payment_method`);
    ...
    `;
    console.log(`âœ… Created index on shipping_city`);
    ...
    console.log(`âœ… Total columns in orders table: ${tableInfo.length}`);

    res.json({
      success: true,
      message: "Database schema setup completed successfully",
      columnsCount: tableInfo.length,
      columns: tableInfo.map(c => `${c.column_name} (${c.data_type})`)
    });
...
app.get("/admin/database-info", verifyToken, async (req, res) => {
  try {
    const userId = req.user?.id;
    
    // Check if user is admin
    const userResult = await sql`SELECT is_admin FROM users WHERE id = ${userId}`;
    if (!userResult || !userResult[0]?.is_admin) {
      return res.status(403).json({ 
        success: false, 
    ...
    // Get order count
    const orderCount = await sql`SELECT COUNT(*) as total FROM orders`;
    ...
    res.json({
      ...
      statistics: {
        totalOrders: orderCount[0]?.total || 0,
...
app.listen(PORT, () => {
    ...
    console.log(`\nðŸ"§ To setup Neon database schema, run:`);
    console.log(`   curl -X POST https://rfashion.online/admin/setup-database \\`);
