...
class AuthProvider extends ChangeNotifier {
  ...
  Future<bool> login(String email, String password) async {
    try {
      ...
      if (response.requiresTwoFactor && response.twoFactorToken != null) {
        _pendingTwoFactorToken = response.twoFactorToken;
        _pendingTwoFactorEmail = response.email ?? email;
        _error = null;
        _isLoading = false;
        notifyListeners();
        return false;
  ...
  Future<void> _completeLogin(AuthResponse response) async {
    ...
    await StorageService.saveToken(response.token!, expiryInSeconds: 172800);
  ...
  Future<bool> signInWithGoogle() async {
    try {
      ...
      if (kDebugMode) {
        print(
            '[AuthProvider] Sending Google auth to backend: ${Constants.baseUrl}/auth/google');
      ...
      if (response.success && response.token != null && response.user != null) {
        ...
        if (kDebugMode) {
          print(
              '[AuthProvider] Google auth successful - saving token: ${response.token!.substring(0, 20)}...');
        ...
        if (kDebugMode) {
          print(
              '[AuthProvider] Token verification after save: ${savedToken != null ? "✅ Success" : "❌ Failed"}');
          if (savedToken != null) {
            print(
  ...
  Future<void> updateLocalAddress(String address) async {
    if (_user != null) {
      _user = _user!.copyWith(address: address);
    }

    await StorageService.saveUserAddress(address);
    notifyListeners();
  }
  ...
  Future<bool> checkAuthStatus() async {
    if (kDebugMode) {
      ...
      print(
          '[AuthProvider] Current token exists: ${_token != null && _token!.isNotEmpty}');
    }
